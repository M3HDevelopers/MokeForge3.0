...
export interface TextBox {
  ...
  fontFamily: string; fontWeight: number; color: string; align: TextBoxAlign;
  bgType: TextBoxBgType; bgColor: string; bgGradient?: string; padding: number;
  borderRadius: number; opacity: number; rotation: number; shadow: boolean; glow: boolean; glowColor: string;
}
...
export type Mood = 'auto' | 'minimal' | 'premium' | 'creative' | 'developer' | 'dark' | 'light' | 'editorial' | 'bold' | 'elegant' | 'futuristic' | 'playful' | 'corporate' | 'luxury' | 'impact' | 'technical';
...
export interface ImageBgState {
  kind: BackgroundKind; imageId: string | null; customSrc: string | null; fit: ImageFit;
  x: number; y: number; scale: number; rotation: number; opacity: number;
  brightness: number; contrast: number; saturation: number; blur: number; hue: number;
  colorFilter: ImageColorFilter; tint: string | null; tintOpacity: number;
  overlay: ImageOverlay; overlayColor: string; overlayOpacity: number;
  blend: GlobalCompositeOperation; mask: ImageMask;
}
...
export interface IconLayer {
  id: string; iconId: string; x: number; y: number; size: number; color: string;
  opacity: number; rotation: number; bgStyle: IconBgStyle; bgColor: string | null;
  shadow: boolean; glow: boolean; material?: IconMaterial; gradient?: { from: string; to: string };
}
...
export interface Background {
  ...
  pattern: PatternKind; patternOpacity: number; style: BgStyle; seed: number;
  light: Lighting; meshPoints: number; kind?: BackgroundKind; image?: ImageBgState;
}
export interface DeviceLayer {
  id: string; kind: DeviceKind; name: string; x: number; y: number; w: number;
  tilt: number; color: string; assetId: string | null; fit: FitMode;
  zoom: number; panX: number; panY: number; shadow: ShadowPreset; url: string;
  visible: boolean; brightness: number; reflection: number; radiusMul: number;
  opacity: number; material: Material; z: number;
}
export interface TextBlock {
  enabled: boolean; title: string; subtitle: string; showBadges: boolean; badges: string[];
  position: PosPreset; scale: number; color: string; autoColor: boolean; fontFamily: string;
  x?: number; y?: number;
}
...
export interface Project {
  ...
  canvas: { w: number; h: number }; assets: Asset[]; devices: DeviceLayer[];
  background: Background; text: TextBlock; logo: LogoState; decoration: DecorationState;
  accents: { a1: string; a2: string }; thumbnail: string | null; exportCount: number;
  decos: DecoLayer[]; mood: Mood; icons: IconLayer[]; textboxes: TextBox[];
}
export interface Selection { kind: 'device' | 'text' | 'logo' | 'background' | 'deco' | 'icon' | 'textbox'; id?: string; ids?: string[]; }
...
export interface DesignSnapshot { id: string; label: string; at: number; thumb: string; score: number; project: Omit<Project, 'assets'>; }
